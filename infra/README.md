...
> Interview soundbite: "Locally I validated RAG end to end with fastembed — the retrieval layer hits recall@3=1.0; the generation layer uses claim-level NLI three-way scoring (supported/neutral/contradicted), with a contradiction rate ≈0.01 (the primary safety metric) and grounding coverage of 0.42 inside the knowledge base vs. 0.01 outside it (acting as an out-of-scope detector that triggers refusal/alerting). Moving to the cloud swaps in pgvector on RDS + a cloud embedding API — the retrieval logic (the shared `services.process_care_plan`) doesn't change by a single line; only the provider and the bootstrap table-creation step change."
